...
import no.skatteetaten.aurora.herkimer.security.AuthState.AUTH_DISABLED
import no.skatteetaten.aurora.herkimer.security.AuthState.AUTH_ENABLED
...
@WebMvcTest(TestController::class, properties = ["aurora.authentication.token.value=secret_from_file"])
class WebSecurityAuroraTokenValueTest @Autowired constructor(mvc: MockMvc) : WebSecurityConfigTest(mvc)

@WebMvcTest(
    TestController::class,
    properties = ["aurora.authentication.token.location=./src/test/resources/secret.txt"]
)
class WebSecurityAuroraTokenLocationTest @Autowired constructor(mvc: MockMvc) : WebSecurityConfigTest(mvc)

@WebMvcTest(
    TestController::class,
    properties = ["aurora.authentication.token.value=secret_from_file", "aurora.authentication.enabled=false"]
)
